# RenPySlim 交接状态（截至 2026-08-25 v0.16.0 发版日）

> 给下一次开工的自己/协作者：读完本页 + BACKLOG.md + ARCHITECTURE.md 即可无缝接手。

## 当前版本：v0.16.0（多语言感知字体瘦身 + 全线并行；v0.10.0 起 AGPL-3.0）

仓库：https://github.com/AxelBeary/renpyslim （公开，AGPL-3.0；v0.10 起由 Apache-2.0 改签，
用户拍板；第三方声明见 THIRD_PARTY_NOTICES.md）
Release：https://github.com/AxelBeary/renpyslim/releases/tag/v0.16.0（自更新检查靶子，
附 exe；发版收尾惯例：验 Release 页面与产物真实存在，不只看流水线状态——
v0.12.0 曾遭 GitHub 503 静默缺失，本次已验证）
回归测试：226 项全绿（`pytest tests -q`，含 test_font_lang 24 条多语言字体瘦身专项；
test_i18n 八本字典互检）

**项目状态：休闲期**（v0.16.0 属核心缺口补齐，用户拍板 2026-08-25“这是必须有的功能”；
