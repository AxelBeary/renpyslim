...
## 2026-08-15 前端整体重构（进行中，已存盘）

用户拍板：完全重构界面，要实用美观、中英双语、亮暗双主题。
方向已拍板：**侧边栏导航 + 暖色圆润风格**（浅色暖白底，暗色为中性暖灰
——第一版暖棕暗色被用户否掉后换的，别再改回棕色）。

已完成：
- 原型 `_tmp/ui_prototype.html` 四组合截图验收通过（亮色一次过，暗色第二版过）
- 正式版 `web/static/index.html` 已全量重写：零依赖单文件，侧边栏+顶栏，
  中英字典即时切换（localStorage 记忆），亮暗双主题（跟随系统+手动，
  首屏防闪），进度条可视化（日志 i/total 驱动），选项分组折叠，结果统计卡片化
- 后端 API 契约零改动；pytest 68 项全绿（后端未动）

**下次接手待办（按顺序）：**
1. 真实后端浏览器实测未做完（被用户中断）：起 uvicorn
   `.venv\Scripts\python -m uvicorn web.app:app --port 8765`，
   用 E:\renpy\JIGSAW_PUZZLES 跑一遍真实分析+执行流程，验证轮询/渲染/进度条；
   中英 x 亮暗四组合截图交用户终验
2. 重建 exe（build_exe.bat 口径）并在 exe 内复查
3. 用户验收后再提交 git（提交时只加代码文件，别误加样本）

已知未决小项：后端返回的日志/警告保持中文原文（计划内边界，英文模式
下日志区为中文）；exe 未重建（当前 dist\RenPySlim.exe 还是旧界面）。
