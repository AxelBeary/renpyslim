# RenPySlim 交接状态（截至 2026-08-28 v0.16.1 发版日）

> 给下一次开工的自己/协作者：读完本页 + BACKLOG.md + ARCHITECTURE.md 即可无缝接手。

## 当前版本：v0.16.1（字体标签扫描精确度修复补丁；v0.10.0 起 AGPL-3.0）
...
## 2026-08-28 v0.16.1 补丁（debug 扫描抓出嵌套标签丢字，用户拍板修复）

debug 扫描压测 8 种边缘写法，发现嵌套 {font=} 标签会把外层字体的字归丢：
内层 {/font} 关闭后，外层字体继续显示的后续文字无归属，精确档剃掉即方框；
另发现 {{转义双花括号}} 被误当真标签乱归属（方向安全但口径不对）。
修复：charset.py 字体归属从正则换成标签栈解析器（_parse_font_spans），
...
起因：用户问"Ren'Py 社区还有什么别的语言"。调研结论：葡语（巴西）圈
性价比最高，土耳其/德/法热度在涨；用户拍板全加。

1. **界面 i18n 4→8 语言**：新增 pt（巴西葡语）/tr/de/fr 四本字典（各 180 键）
   + LOG_PATTERNS 每模式四语模板（31×4）+ LANGS 注册表 + html lang 映射
   （zh→zh-CN、pt→pt-BR，其余用语言码本身）。
2. **文档八件套**：新增 README.pt/tr/de/fr.md（中文 README 为事实源）；
   全部 8 份 README 的切换行/语言清单/多语言表格同步；CONTRIBUTING
   翻译指南同步为八本字典口径。
3. **顺带修正**：各 README 测试数 77→114；中文 README AI 免责行笔误
   （“深度拉屎”→“AI 深度参与开发”，已告知用户可否决）。
- 做法：四个并行翻译子代理产出，主代理写合并脚本程序化注入
  （正则逐条比对防盲合），node 实测加载验证
- 质量：test_i18n LANGS 元组扩到 8；114 项测试全绿；葡语字典经
  西语混入抽查无泄漏
- 待办：无；翻译未经母语者审校，若社区反馈用词问题再精修
...
## 2026-08-25 多语言感知字体瘦身 + 全线并行（补第一天起的缺口）
发布版本号：**v0.16.0**

起因：用户问“能不能按语言识别字体并定向清理”。查实：字符集提取从第一天起就把所有语言混成一锅，
单语言专用字体也背着全部语言的字形。用户拍板：这是核心缺口不是新功能，必须做。同日六轮迭代：
功能落地 → 定位修正 → 两轮深挖审查修红线 → 优化批 → 字体并行化 → 终审收口。

1. **多语言感知字体瘦身**：
   - charset.py：按翻译语言分桶，`scan_charset_tables` 一遍扫描产出语言桶 / {font=} 标签
     归属桶 / 插值污染集 / 动态输入清单；成品模式跳过 renpy/、lib/ 引擎目录，但 renpy/common
     内置界面用字（菜单/按键帮助，未翻译直接上屏）定向收集强制保留
   - pipeline.py：三档字体瘦身计划 `_font_slim_plan`——precise（全引用在闭合标签内且无插值）/
     lang_scoped（引用只出现在部分语言的 tl 目录，强制并入主剧本桶）/ global 兜底；
     缺字对账与瘦身共用同一计划（压假警报且口径一致）
   - 报告新增 languages / font_usage（含语言归属）/ font_slim_modes 字段
2. **用户拍板修正**：曾加过单语言发行过滤（--language / 界面下拉框 / 仅主剧本），同日移除——
   用本工具的前提就是发多语言大包，字符集永远全语言合集；定向优化靠逐字体而非全局过滤。
   教训：产品定位纠偏优先于既有设计，沉没成本不是保留理由。
3. **三轮深挖审查（双路子代理交叉验证）修红线**：主字体（只在 tl 外引用）禁止收窄；
   插值 / 注释 / 未闭合 / 同行混合引用不得作精确证据；rpy/rpyc 不一致对精确档与定向档
   都降级全量；编译兜底计数改边界正则防超计——每条都配红线回归测试。
   教训：引用的静态坐标只是服务范围的必要证据非充分证据；审查报告须逐条对照源码核实。
4. **优化批与并行**：工程线字体瘦身并行化（复用 _run_jobs 同款框架，计划预先定好任务只执行），
   工程/成品全线并行残余债清零；文本扫描三遍合一；无引用检测补大小写盲区（防误隔离）；
   编译文本缓存加锁 + 按修改时间/文件数失效。
5. **发版**：预创建手写中文说明 Release + 标签触发流水线（容忍逻辑保留说明只补传 exe），
   2m04s 全绿，31.2MB exe 已验证真实存在。同日另按既定策略关闭两个 Dependabot 纯版本抬升 PR
   （经仓库告警 + GHSA 公告 + 实际安装版本三重核查确认无安全动机）。

测试基线：202 → **226 项全绿**（新增 test_font_lang 24 条专项）；ruff --select F,E9 全绿；
4 个 2.9MB 真实大字体并行瘦身端到端实测（共省 11.5MB，零失败）。
